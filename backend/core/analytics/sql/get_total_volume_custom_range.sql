...
-- plan_volume_kg backs the "vs Plan" column for the same date range.
SELECT
  exercise_id,
  SUM(volume) FILTER (WHERE scenario = 'actuals') AS total_volume_kg,
  SUM(volume) FILTER (WHERE scenario = 'plan') AS plan_volume_kg
